...
export class BuypetProvider {
        ...
        this.db.collection('users').get().then(user => {
          user.docs.map(user => {
            ...
            if (email != localStorage.getItem('email')) {
              ...
              firebase.database().ref(`notif/adopt/${userName}/`).once('value', snapshot => {
                console.log('firing')
                if (snapshot.hasChild('unread')) {
                  let unread = snapshot.val().unread + 1;
                  firebase.database().ref(`notif/adopt/${userName}/`).set({
                    unread: unread
                  });
                } else {
                  firebase.database().ref(`notif/adopt/${userName}/`).set({
  ...
  saveUpdatedAdoptPetInfo(pet: object, petId: string) {
    const promise = new Promise((resolve, reject) => {
      pet['datePosted'] = firebase.firestore.FieldValue.serverTimestamp();
